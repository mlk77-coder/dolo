...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Rating" />
    <x-common.component-card title="Edit Rating">
        <form action="{{ route('ratings.update', $rating) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="user_id" required class="w-full px-4 py-2 border rounded-lg">
                        ...
                        @foreach(\App\Models\User::all() as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $rating->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="deal_id" required class="w-full px-4 py-2 border rounded-lg">
                        ...
                        @foreach($deals as $deal)
                            <option value="{{ $deal->id }}" {{ old('deal_id', $rating->deal_id) == $deal->id ? 'selected' : '' }}>{{ $deal->title_en }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="stars" required class="w-full px-4 py-2 border rounded-lg">
                        @for($i = 1; $i <= 5; $i++)
                            <option value="{{ $i }}" {{ old('stars', $rating->stars) == $i ? 'selected' : '' }}>{{ $i }} Star{{ $i > 1 ? 's' : '' }}</option>
                        @endfor
                    </select>
                ...
                <div>
                    ...
                    <select name="approved" class="w-full px-4 py-2 border rounded-lg">
                        <option value="0" {{ old('approved', $rating->approved) == '0' ? 'selected' : '' }}>No</option>
                        <option value="1" {{ old('approved', $rating->approved) == '1' ? 'selected' : '' }}>Yes</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="comment" rows="4" class="w-full px-4 py-2 border rounded-lg">{{ old('comment', $rating->comment) }}</textarea>
