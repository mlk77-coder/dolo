...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Redemption" />
    <div class="space-y-6">
        <x-common.component-card title="Edit Redemption">
            <form action="{{ route('redemptions.update', $redemption) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="order_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                            ...
                            @foreach($orders as $order)
                                <option value="{{ $order->id }}" {{ old('order_id', $redemption->order_id) == $order->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="user_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ old('user_id', $redemption->user_id) == $customer->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="merchant_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                            ...
                            @foreach(\App\Models\Merchant::all() as $merchant)
                                <option value="{{ $merchant->id }}" {{ old('merchant_id', $redemption->merchant_id) == $merchant->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                            <option value="pending" {{ old('status', $redemption->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="completed" {{ old('status', $redemption->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                            <option value="cancelled" {{ old('status', $redemption->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-theme-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="redeemed_at" value="{{ old('redeemed_at', $redemption->redeemed_at ? $redemption->redeemed_at->format('Y-m-d\TH:i') : '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="redeemed_by" value="{{ old('redeemed_by', $redemption->redeemed_by) }}"
                ...
                <div>
                    ...
                    <textarea name="notes" rows="4"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('notes', $redemption->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-theme-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
