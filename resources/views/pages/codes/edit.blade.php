    ...
    <x-common.component-card title="Edit Discount Code">
        <form action="{{ route('codes.update', $code) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="subject" value="{{ old('subject', $code->subject) }}" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500" placeholder="e.g., Summer Sale 2024">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $code->code) }}" required class="w-full px-4 py-2 border rounded-lg font-mono uppercase focus:ring-2 focus:ring-brand-500 focus:border-brand-500" placeholder="SUMMER20" style="text-transform: uppercase;">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="discount_amount" value="{{ old('discount_amount', $code->discount_amount) }}" required step="0.01" min="0" class="w-full pl-8 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500" placeholder="10.00">
                    </div>
            ...
            <div>
                ...
                <input type="date" name="valid_to" value="{{ old('valid_to', $code->valid_to?->format('Y-m-d')) }}" min="{{ date('Y-m-d') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            ...
            <div>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $code->is_active) ? 'checked' : '' }} class="w-5 h-5 text-brand-500 border-gray-300 rounded focus:ring-brand-500">
            ...
            <div>
                ...
                <input type="url" name="external_url" value="{{ old('external_url', $code->external_url) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500" placeholder="https://example.com/deal">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">PNG, JPG, WEBP, max 2MB. Leave empty to keep current image.</p>
